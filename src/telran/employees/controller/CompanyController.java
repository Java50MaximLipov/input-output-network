...
public class CompanyController {
	...
	static void addEmployee(InputOutput io) {
		...
		LocalDate birthDate = io.readIsoDate("Enter birtdate in ISO format", "Wrong birthdate", getBirthdate(MAX_AGE),
	...
	static void getSalaryDistribution(InputOutput io) {
		int interval = io.readInt("Enter salary distribution interval", "Wrong interval", MIN_INTERVAL, MAX_INTERVAL);
	...
	static void updateDepartment(InputOutput io) {
		...
		io.writeLine(String.format("old deprtment %s of employee %d" + " has been updated with department %s",
				empl.department(), empl.id(), department));
	}
	...
	private static int[] getSalaries(InputOutput io) {
		int from = io.readInt("Enter salary from", "Wrong salary-from value", MIN_SALARY, MAX_SALARY - 1);
	...
	private static String getDepartment(InputOutput io) {
		return io.readString("Enter department " + Arrays.deepToString(DEPARTMENTS), "Wrong department",
